...
qplot(age.grp,face,      
      ...
      position=position_jitter(.15),ylim=c(0,.25),
...
qplot(age.grp,face,colour=posture,
      facets = .~ orientation,
            ymin=face - f.cil, ymax = face + f.cih,
            geom=c("line","pointrange"),
      position=position_dodge(width=.1,height=0),ylim=c(0,.25),
      xlab="Age (Months)",ylab="Proportion Faces Detected",
      data=subset(ms,orientation!="other")) +
  scale_x_continuous(breaks=c(8,12,16))
